...
def create_navigation(slide_index, total_slides):
    """Create navigation HTML for a slide with color-coded agenda section"""
    current = SLIDES[slide_index]
    agenda_section = current.get('agenda_section', 'General')
    agenda_color = AGENDA_COLORS.get(agenda_section, '#666')
    ...
    # Navigation HTML with agenda section after Previous button
    nav_html = f'''
    <nav class="slide-navigation">
        <div class="nav-left">
            {prev_link}
            <span class="nav-agenda" style="background: {agenda_color};">{agenda_section}</span>
        </div>
...
def add_navigation_css():
    """CSS for the navigation bar with agenda section styling"""
    return '''
    <style>
        .slide-navigation {
            position: fixed;
...
def create_index_page(output_dir):
    """Create the index/contents page with color-coded agenda sections"""
...
'''
    ...
    # Add table of contents entries - simple numbered list
    for slide in SLIDES:
        index_html += f'''
            <li class="toc-item">
